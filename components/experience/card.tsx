          ...
          <div className={"grid grid-cols-[1fr_auto] gap-4"}>
            ...
            {experience.endDate ? (
              <Badge
                ...
                variant={"outline"}
              >
                {format(experience.startDate, 'MM/yyyy')} – {format(experience.endDate, 'MM/yyyy')}
              </Badge>
            ) : (
              <Badge
                className={"mt-4 mr-4 rounded-full h-fit bg-green-100 dark:bg-green-950 border-green-400 dark:border-green-700"}
                variant={"outline"}
              >
              ...
              <div className={"grid grid-cols-[auto_1fr] gap-2"}>
                ...
                <div className={"flex flex-wrap gap-2"}>
                  {experience.tools.map((tool) => (
                    <Badge key={tool} variant={"outline"} className={"bg-muted/50 rounded-full h-fit"}>
                      {tool}
                    </Badge>
                  ))}
                </div>
